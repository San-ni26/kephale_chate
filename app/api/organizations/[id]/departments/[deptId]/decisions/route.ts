...
export async function GET(
    ...
    try {
        ...
        return NextResponse.json({ decisions: decryptUserPII(decisions) });
    } catch (error) {
...
export async function POST(
    ...
    try {
        ...
        if (!access || (!access.isOrgAdmin && !access.isDeptHead)) {
            return NextResponse.json({ error: 'Seuls les admins ou le chef du département peuvent proposer des décisions' }, { status: 403 });
        }

        const body = await request.json();
        const { title, description, voteDeadline } = body;

        if (!title) {
            return NextResponse.json({ error: 'Titre requis' }, { status: 400 });
        }

        const decision = await prisma.teamDecision.create({
            data: {
                deptId,
                title,

...
export default function DepartmentChatPage() {
    ...
    const sendAudioMessage = async (audioFile: File) => {
        ...
        let audioUrl: string;
        try {
            audioUrl = await uploadFileToSupabase(audioFile, 'audio', deptId || 'unknown');
        } catch (err) {
    ...
    const handleSendMessage = async () => {
        ...
        if (currentFiles.length > 0) {
            for (const file of currentFiles) {
                ...
                const fileUrl = await uploadFileToSupabase(file, fileType === 'AUDIO' ? 'audio' : 'messages', deptId || 'unknown');

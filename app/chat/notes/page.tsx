...
function NotesPageContent() {
    ...
    /* sélection groupe : synchro avec URL (TopNav affiche le sélecteur) */
    const selectedGroupId = groupFromUrl;
    ...
    /* rediriger vers URL avec 1er groupe si aucun groupe dans l'URL */
    useEffect(() => {
        if (!groupFromUrl && groups.length > 0) {
            router.replace(`/chat/notes?group=${groups[0].id}`);
        }
    }, [groups, groupFromUrl, router]);
    ...
    const shareNoteWithUser = async (note?: Note | null) => {
        const targetNote = note ?? shareNote;
        if (!selectedGroupId || !defaultDocId || !targetNote || !shareEmail.trim()) return;
        setShareLoading(true);
        try {
            const res = await fetchWithAuth(
                `/api/groups/${selectedGroupId}/documents/${defaultDocId}/notes/${targetNote.id}/share`,
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
            ...
            const refreshed = updated.find((n: Note) => n.id === targetNote.id);
            if (viewNote && targetNote && refreshed) setViewNote(refreshed);
            if (shareNote && targetNote && refreshed) setShareNote(refreshed);
            if (editingNote && targetNote && refreshed) setEditingNote(refreshed);
        } catch (e) {
    ...
    const unshareNote = async (sharedWithId: string, note?: Note | null) => {
        const targetNote = note ?? shareNote;
        if (!selectedGroupId || !defaultDocId || !targetNote) return;
        const share = targetNote.shares?.find((s) => s.sharedWith.id === sharedWithId);
        if (!share) return;
        setShareLoading(true);
        try {
            const res = await fetchWithAuth(
                `/api/groups/${selectedGroupId}/documents/${defaultDocId}/notes/${targetNote.id}/share?sharedWithId=${encodeURIComponent(sharedWithId)}`,
                { method: "DELETE" }
            );
            if (!res.ok) {
            ...
            const updated = await loadNotes(selectedGroupId);
            const refreshed = updated.find((n: Note) => n.id === targetNote.id);
            if (shareNote && targetNote) setShareNote(refreshed ?? null);
            if (viewNote && targetNote && refreshed) setViewNote(refreshed);
            if (editingNote && targetNote && refreshed) setEditingNote(refreshed);
        } catch (e) {
            toast.error("Erreur");
    ...
    const updateSharePermission = async (sharedWithId: string, canEdit: boolean, note?: Note | null) => {
        const targetNote = note ?? shareNote;
        if (!selectedGroupId || !defaultDocId || !targetNote) return;
        setShareLoading(true);
        try {
    ...
    const createGroup = async () => {
        ...
        try {
            ...
            await mutateGroups();
            if (data.group?.id) {
                router.push(`/chat/notes?group=${data.group.id}`);
            }
        } catch (e) {
    ...
    const confirmDeleteGroup = async () => {
        ...
        try {
            ...
            if (selectedGroupId === g.id) {
                ...
                router.push(nextId ? `/chat/notes?group=${nextId}` : "/chat/notes");
            }
            await mutateGroups();
        } catch (e) {
    ...
    return (
        <div className="flex flex-col min-h-screen sm:min-h-0 sm:h-full pt-14 sm:pt-16 pb-20 sm:pb-4">
            {/* ── header (titre + sélecteur groupe dans TopNav) ── */}
            ...
            <Dialog open={formOpen} onOpenChange={setFormOpen}>
                <DialogContent className="bg-card border-border text-foreground w-[95vw] sm:w-full max-w-2xl max-h-[85vh] sm:max-h-[90vh] flex flex-col">
                    ...
                    <div className="space-y-4 flex-1 overflow-y-auto min-h-0 pr-1">
                        ...
                        {/* Section Partager (uniquement en édition, pour le créateur) */}
                        {!editingNote && (
                            <p className="text-xs text-muted-foreground pt-2 border-t border-border">
                                Enregistrez la note pour la partager avec d&apos;autres membres du groupe.
                            </p>
                        )}
                        {editingNote && selectedGroupId && defaultDocId && editingNote.creator?.id === currentUserId && (
                            <div className="pt-4 border-t border-border space-y-3">
                                ...
                                <p className="text-xs text-muted-foreground">
                                    Partagez avec un membre du groupe par email. Choisissez s&apos;il peut modifier ou seulement lire.
                                </p>
                                <div className="flex gap-2">
                                    ...
                                    <Button
                                        size="sm"
                                        onClick={() => shareNoteWithUser(editingNote)}
                                        disabled={shareLoading || !shareEmail.trim()}
                                    >
                                        {shareLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Ajouter"}
                                    </Button>
                                </div>
                                {editingNote.shares && editingNote.shares.length > 0 && (
                                    <div className="space-y-2">
                                        <div className="text-xs font-medium text-muted-foreground">Partagée avec :</div>
                                        <div className="space-y-1.5 max-h-32 overflow-y-auto">
                                            {editingNote.shares.map((s) => (
                                                <div
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center gap-1 shrink-0">
                                                        <Button
                                                            ...
                                                            onClick={() => updateSharePermission(s.sharedWith.id, !(s.canEdit ?? true), editingNote)}
                                                        ...
                                                        <Button
                                                            ...
                                                            onClick={() => unshareNote(s.sharedWith.id, editingNote)}
                                                            disabled={shareLoading}
...
export default function NotesPage() {
    return (
        <Suspense fallback={
            <div className="flex flex-col min-h-screen pt-14 sm:pt-16 pb-20 sm:pb-4">
                <div className="flex items-center justify-center py-20">
                    <Loader2 className="w-8 h-8 animate-spin text-muted-foreground" />
                </div>
            </div>

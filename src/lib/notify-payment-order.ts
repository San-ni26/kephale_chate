...
export type PaymentOrderType = 'CREATE' | 'UPGRADE' | 'USER_PRO' | 'DISCUSSION_RIGHTS';
...
export async function notifySuperAdminNewPaymentOrder(params: NotifyPaymentOrderParams): Promise<void> {
    ...
    try {
        ...
        const typeLabel = type === 'UPGRADE' ? 'Mise à niveau d\'abonnement' : type === 'USER_PRO' ? 'Compte Pro' : type === 'DISCUSSION_RIGHTS' ? 'Droits de discussion' : 'Nouvelle organisation';
